...
fn main() {
    ...
    // Use time_step_mut for more efficient in-place updates
    // This avoids an extra allocation compared to time_step()
    for i in 0..20 {
        println!("time step: {}", i);
        test_tree.time_step_mut();
    }

    println!("Final entity count: {}", test_tree.get_number_of_entities());
}
